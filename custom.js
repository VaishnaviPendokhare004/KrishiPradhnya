...
document.addEventListener('DOMContentLoaded', function() {
    const viewDetailsButtons = document.querySelectorAll('.new-cars-btn');
    
    viewDetailsButtons.forEach((button, index) => {
        button.addEventListener('click', function(e) {
            e.preventDefault();
            
            // Map index to technology ID
            const techIds = ['vertical-farming', 'precision-farming', 'marginal-farmers', 'hydroponic'];
            const techId = techIds[index];
            
            // Redirect to technology details page with ID
            window.location.href = `technology-details.html?id=${techId}`;
        });
    });
});
